...
🟢 **2026-09-08 站主在演示空间实测到了这个行为**：建任务、执行者设成 Bot、
点「立即执行」，它执行完**直接停在「已完成」**，中间没有任何审查环节，
界面也没有任何提示说少了一步。**但归因未分离**——观察到的是行为，
「原因是状态配置里缺那一档」这半句仍未验证。要分离得再跑一次：
先确认状态配置里有没有「人工审查」，有则加上再跑对照。
写文章时口径只能到「审批点不是默认就有的，先跑一次看它停在哪儿」。
